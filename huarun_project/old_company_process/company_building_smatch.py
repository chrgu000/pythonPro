...
import json
from  no_match_company import get_no_match_company_info

def get_match_building():
    company_list=get_no_match_company_info()
    c_match_building=[]
    for i in range(0,len(company_list)):
        ...
        if m_name!="":
            c_match_building.append(c_id+"%"+str(m_name)+"%"+str(min_dis)+"%"+str(m_addr))
            print(c_id+"%"+str(m_name)+"%"+str(min_dis)+"%"+str(m_addr))
    return c_match_building
...
if __name__=='__main__':
    c_match_buinding=get_match_building()
    pass